...
## 8. 英文全量实测（官方 dump 源）——**当前交付版**

**日期**：2026-09-14
**输入**：`data/enwiki-latest-pages-articles.xml.bz2`（23.9 GB，2026-09 快照）
**产物**：`out/wikipedia-en.mobi`

解析 dump（1 小时 55 分）：

```
词条 6,877,357 条 → source.tsv（2.0 GB）
异名 11,530,630 条 → aliases.tsv
```

归一 + 显著度筛选 + 构建（4 分 12 秒）：

```
阶段 4    归一补消歧义别名 +73,326 条 → 合并后异名 11,603,956 条
阶段 4.5  按跨语言链接数 ≥ 9 筛选：6,877,357 → 1,038,531 条
          异名表同步过滤 → 4,045,999 条
阶段 5    预估 106,005 条记录（超限）→ 自动截断释义至 200 字 → 52,610 条记录
阶段 6    并入异名 3,971,866 条；正文 466.1 MB / 未压缩 218.9 MB
          Compressed text into 53434 records；Orth INDX 3304
          Wrote out/wikipedia-en.mobi (314408945 bytes = 299.8 MB)
阶段 7    全部通过（rec_count 56742）
系数校准  en = 3296.4
```

**这条链是自动跑的**：解析完成判据为「source.tsv 连续 3 分钟不增长」，
之后自动接上归一 → 筛选 → 构建 → 验收，全程无需人工介入（验证了工具链可以无人值守跑长任务）。

**与英文第一版对比（第一版用 DBpedia 2022 摘要、无归一）**：

| | 第一版 | 本版 |
| --- | --- | --- |
| 数据来源 | DBpedia 2022 摘要 | enwiki 官方 dump 2026-09 |
| 词条 | 984,339 | **1,038,531** |
| 异名 | 0 | **3,971,866**（重定向 + 归一） |
| 体积 | 144.9 MB | 299.8 MB（异名推高索引） |

## 10. 最严重的一次故障：英文版真机查不到词（2026-09-15）

**现象**：用户侧载英文正式版后反馈「查不到单词」。

**排查过程**（每一步都值得记）：

1. **先查元数据** —— `locale` / `dict_input_lang` / `orth_index` / `EXTH[105]` 全部正常。
2. **再查索引能不能匹配** —— 用 `kindling lookup` 模拟设备查词，扫了 95 个常见词与专有名词
   （含用户举的 `Mars`、`Earth`），**全部解析成功**。这一步几乎把我带偏。
3. **回头看构建日志** —— 发现一条我在验收时**漏掉的警告**：

```
MOBI check: 16 P0 checks passed, 1 P1 warnings
(INDX record 53435: entry 1482 of 3299 starts at offset 43, out of order or outside the
 entries; an index record past 64 KB wraps its 16-bit IDXT offsets)
```

4. **解剖索引记录**（`kindling dump` 给出每条 INDX 的 `length` 与 `idxt_offset`）：

| | INDX 记录数 | 记录最大长度 | **IDXT 偏移最大** |
| --- | --- | --- | --- |
| 中文（正常） | 1,262 | 64,196 B | 24,721 ✓ |
| 英文（正式版） | 3,304 | **196,210 B** | **141,138 ✗ 溢出** |
| 英文第一版（无别名，正常） | 670 | 64,196 B | 27,214 ✓ |

**根因**：MOBI 的 `idxt_offset` 是 **16 位（上限 65,535）**。词典的**根索引**要为每个叶子索引
存一条记录，而叶子数 ≈ 总键数 ÷ 每叶约 1400 个键。英文的键 = 104 万词头 + 397 万别名 = **468 万**，
把叶子数推到 3,299，根索引长到 196 KB —— 偏移量溢出，**设备读不到根索引表，整本查不到任何词**。

**为什么验收没拦住**：`kindling` 自己的 `dump`/`lookup` 能正确处理 >64 KB 的索引记录，
`lookup` 甚至会报告"解析成功"。**这个缺陷只有真机会暴露**——而我当时查的是"索引存在"，
没查"索引记录有没有超 64 KB"，也没把构建日志里的 P1 警告当失败。

### 修复

键长是唯一的杠杆——它同时减少叶子数和每条根索引的大小。实测（键长上限 → 根索引偏移）：

| 键长上限 | 词头 | 总键数 | 根索引 IDXT 偏移 | 结果 |
| --- | --- | --- | --- | --- |
| 无限制 | 1,038,531 | 4,680,000 | 141,138 | ❌ 溢出 |
| 28 | 947,786 | 4,269,832 | 88,428 | ❌ 溢出 |
| 24 | 889,340 | 3,798,894 | 70,269 | ❌ 溢出 |
| **20** | **781,488** | **2,927,007** | **61,613** | ✅ **安全** |

**上限必须卡在 20 字符**，已做成工具选项 `--max-key-len`。
这个限制**不损失实用内容**——Kindle 单次查词约 4 个词封顶（见 [docs/07](07-automation.md)），
超长标题本来就查不到。

### 修复后

```
词条 781,488 条   并入异名 2,249,542 条   键 2,927,007
Orth INDX 1,645 条   最大 IDXT 偏移 61,613 ✓
MOBI check: 16 P0 通过，0 P1 警告
out/wikipedia-en.mobi = 244.4 MB
```

### 验收已加固（防止再犯）

`wiki2kindle.py` 的验收从 4 项加到 **6 项**：

- 新增 **`索引记录未超 64KB（否则设备读不到）`**：解析全部 INDX，判据是 `idxt_offset ≤ 65,535`
  （不是记录长度——记录可以超过 64 KB，只要索引表落在 64 KB 以内）
- 新增 **`MOBI 自检无 P1 警告`**：把构建日志里的 P1 计数当失败，不再只看 P0

**余量提醒**：修复后中文最大偏移 62,273、英文 61,613，**都只剩约 5% 余量**。
换更新的 dump 重建时必须重新核对这一项。

## 11. 真机验证（Paperwhite 11/12 代）

**已通过（2026-09-14 用户反馈）**：

- ✅ 侧载成功，出现在「设置 → 语言和字典 → 字典」中
- ✅ 选词能弹出本词典的释义，初步使用体验「挺好的」（323 MB / 128 万条）

这是整条链路第一次走完真机闭环——格式、元数据、索引、侧载全部成立。

**仍待细验**：

- [ ] **繁体/简体异名命中**：简体书里选「信义区」应命中 `信義區 (臺北市)`；繁体书里选「信義區」同理
- [ ] 323 MB / 128 万条下的查词响应速度（是否明显卡顿）
