...
| **英文全量** | ✅ **已交付**：`out/wikipedia-en.mobi`，**781,488 条 / 244 MB**，2026-09 内容，含 225 万条异名。从 690 万条中按「跨语言链接数 ≥ 9」筛选，并把**键长限制到 20 字符**（见 [docs/06](docs/06-verification.md) §10——不限制会让根索引溢位、设备读不到索引表）。**单本装下** |
...
| **记录数**（比体积更紧的约束） | MOBI 的 PalmDB 记录数是 16 位，硬上限 **65,535**；`记录数 ≈ 未压缩正文字节 ÷ 8192`（`idx:` 标记不占正文记录）。据此单本上限约 **158 万条（中文）/ 63 万条（英文）**。 |
| **索引偏移**（更隐蔽，只有真机会暴露） | 索引里 `idxt_offset` 也是 **16 位**。根索引要为每个叶子索引存一条记录，叶子数 ≈ 总键数 ÷ 1400，**键太多就会溢出，设备读不到索引表、整本查不到词**。实测英文键长限制到 20 字符才能落在安全区，见 [docs/06](docs/06-verification.md) §10。 |
| **体积** | 已知能正常使用的商业词典约 118–281 MB；本项目实测 **317 MB 可成功编译**（真机未验证）。 |
| **查词机制** | Kindle 按**词头精确匹配**（外加变形词表）。"广东省"能查到，"广东省经济发展"查不到。中文还多一层坑：简繁与地区词差异必须靠变形词表补进去。**消歧义页**（如「卡」）不会只取首段，而是编成带编号的**多义释义**，弹窗里能一次看到全部义项。 |
...
| **多义词（消歧义页）怎么办** | 检出消歧义页（模板 / `__DISAMBIG__` / 分类 / 「……可以指：」+ 列表兜底），把各义项抽成**带编号的多义释义**；跳过「参见」段，每页最多 10 义项、每项 100 字。带 `(消歧义)` 后缀的标题仍按旧逻辑剔除并把基名让给主条目 |
