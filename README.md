...
| 环节 | 状态 |
| --- | --- |
| 工具链与格式 | ✅ **已验证**——样例词表已完整编译出 MOBI，被正确识别为词典，28 个查词键（词头 + 异名）全部建入索引，无词头定位错误。详见 [docs/06](docs/06-verification.md) |
| 数据获取 | ⏳ 待下载，清单见 [docs/00](docs/00-download-list.md) |
| 中文全量 | ⏳ 待构建与量体积 |
| 英文全量 | ⏳ 待构建；预估超设备承受上限，需按 [docs/05](docs/05-scaling-english-plan.md) 决定是否降级 |
...
| **体积** | 已知能正常使用的最大 Kindle 词典约 **280 MB**。中文全量（约 140 万条）落在 200–300 MB，可行；英文全量（约 690 万条）估算 1.5–2.5 GB，**是已知上限的 7 倍以上，必须先实测再决定是否降级**。 |
