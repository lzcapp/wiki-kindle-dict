...
| 环节 | 状态 |
| --- | --- |
| 工具链与格式 | ✅ **已验证**——样例、中文全量、英文小样三个产物均编译成功，被识别为词典，索引正确。详见 [docs/06](docs/06-verification.md) |
| 数据获取 | 🚧 进行中——DBpedia 中/英摘要已就位；两个官方 dump 下载中 |
| 中文全量 | ✅ **已构建**：1,124,787 条 → **317.1 MB**，但记录数已占用上限的 96.5%（DBpedia 2022 源，重定向异名尚未并入） |
| 英文全量 | ⛔ **单本结构上不可行**——690 万条需要约 11 卷；分卷是否可用取决于 Kindle 是否回退查询其他词典，见 [docs/05](docs/05-scaling-english-plan.md) |
...
| **记录数**（比体积更紧的约束） | MOBI 的 PalmDB 记录数是 16 位，硬上限 **65,535**。中文全量实测已用掉 **96.5%**。据此推算单本上限约 **117 万条（中文）/ 63 万条（英文）**——**英文全量 690 万条必须分卷**。 |
